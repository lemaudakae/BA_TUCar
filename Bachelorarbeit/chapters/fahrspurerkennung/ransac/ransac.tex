...
y	\\
\end{pmatrix}
\pm l \cdot \gls{lat:dv}  
\label{eq:maske_randpunkte}
\end{eqnarray}

Die Normierung von \gls{lat:dv} multipliziert mit der bekannten Fahrspurbreite \( l \) addiert auf den Mittellinienpunkt \( (x,y)^T \) ergibt einen Stützpunkt \( (x_s,y_s)^T \) der linken (\( + \)) bzw. der rechten (\( - \)) Randlinie (Gleichungen~\ref{eq:maske_verschiebungsvektor} und~\ref{eq:maske_randpunkte}).
