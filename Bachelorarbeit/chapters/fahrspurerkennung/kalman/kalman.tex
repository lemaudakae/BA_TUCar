...
\gls{y}(\gls{x}) =
\gls{lat:cfpa} +
\gls{lat:cfpb} \cdot \gls{x} +
\gls{lat:cfpc} \cdot \gls{x}^2 +
\gls{lat:cfpd} \cdot \gls{x}^3
\end{equation}
\begin{equation}
...
Der Einfluss des Lenkwinkels wird vernachlässigt, da er bei kurzer \glsdesc{lat:periodictime} \gls{lat:periodictime} und kleiner Größe seiner selbst kaum Einfluss auf die Lage der Fahrbahn im Folgebild nimmt. Das Fahrzeug bewegt sich also geradlinig in Richtung der x-Achse des Fahrspurkoordinatensystems \gls{lat:LinienKOS}. 

\subsubsection{Herleitung der \glsdesc{lat:systemmatrix}}
Der zurückgelegte Weg \begin{math} \gls{lat:distpic}_{\gls{lat:LinienKOS}} \end{math} zwischen zwei Bildaufnahmenpunkten kann direkt aus den Encodern des Fahrzeugs ausgelesen werden. Der Folgezustand  \(\gls{lat:statevector}(\gls{lat:iter}+1)\) wird wie folgt berechnet:
\begin{equation}
\label{eq:nextstatemapping}
\hat{\gls{lat:statevector}}(\gls{lat:iter}+1) =
\begin{pmatrix}
\gls{y}_{\gls{lat:LinienKOS}\gls{lat:iter}}(\gls{lat:distpic}_{\gls{lat:LinienKOS}}) \\
\der{\gls{y}_{\gls{lat:LinienKOS}\gls{lat:iter}}}(\gls{lat:distpic}_{\gls{lat:LinienKOS}}) \\
\derII{\gls{y}_{\gls{lat:LinienKOS}\gls{lat:iter}}}(\gls{lat:distpic}_{\gls{lat:LinienKOS}}) \\
\derIII{\gls{y}_{\gls{lat:LinienKOS}\gls{lat:iter}}}(\gls{lat:distpic}_{\gls{lat:LinienKOS}}) \\
\end{pmatrix}
\end{equation}

In \autocite{petersfalkoFPGAbasierteBildverarbeitungspipelineZur2009} wird beschrieben, dass sich aufgrund von Nichtlinearität aus \ref{eq:nextstatemapping} keine äquivalente Matrix A aufstellen lässt. Da diese Nichtlinearität jedoch nur in Bezug auf  \gls{lat:distpic} und nicht den \glsdesc{lat:statevector} \gls{lat:statevector} besteht, ist dies trotz dessen möglich. Aus \eqref{eq:polylanestate}, \eqref{eq:nextstatemapping} und der Bewegung des Fahrzeugs entlang der x-Achse des Fahrspurkoordinatensystems \gls{lat:LinienKOS} um \gls{lat:distpic} folgt:
\begin{equation}
\begin{split}
...
\begin{pmatrix}
\gls{lat:lateraloffset} +
\gls{gre:yawangle} \cdot \gls{lat:distpic} +
\frac{1}{2} \cdot \gls{lat:curvature} \cdot (\gls{lat:distpic})^2 +
\frac{1}{6} \cdot \gls{lat:curvaturechange} \cdot (\gls{lat:distpic})^3 \\
\gls{gre:yawangle} + \gls{lat:curvature} \cdot \gls{lat:distpic} +
\frac{1}{2} \cdot \gls{lat:curvaturechange} \cdot (\gls{lat:distpic})^2 \\
\gls{lat:curvature} + \gls{lat:curvaturechange} \cdot \gls{lat:distpic} \\
\gls{lat:curvaturechange}
\end{pmatrix} \\
& \begin{pmatrix}
1 &  \gls{lat:distpic} & \frac{1}{2} \cdot (\gls{lat:distpic})^2 & 
\frac{1}{6} \cdot (\gls{lat:distpic})^3 \\
0 & 1 &  \gls{lat:distpic} & \frac{1}{2} \cdot (\gls{lat:distpic})^2 \\
0 & 0 & 1 &  \gls{lat:distpic} \\
0 & 0 & 0 & 1
\end{pmatrix}
\cdot
\begin{pmatrix}
...
\hat{\gls{lat:statevector}}(\gls{lat:iter}+1) =
\begin{pmatrix}
\gls{lat:lateraloffset} + \gls{gre:yawangle} \cdot \gls{lat:distpic} \\
\gls{gre:yawangle} + \gls{lat:curvature} \cdot \gls{lat:distpic}\\
\gls{lat:curvature} + \gls{lat:curvaturechange} \gls{lat:distpic} \\
\gls{lat:curvaturechange}
\end{pmatrix}
=
\begin{pmatrix}
1 &  \gls{lat:distpic} & 0 & 0 \\
0 & 1 &  \gls{lat:distpic} & 0 \\
0 & 0 & 1 &  \gls{lat:distpic} \\
0 & 0 & 0 & 1
\end{pmatrix}
\cdot
...
Da sich die gefahrene Strecke \begin{math} \gls{lat:distpic}_{\gls{lat:LinienKOS}} \end{math} zwischenden Aufnahmepunkten zweier Bildern geringfügig ändern kann, muss die \glsdesc{lat:systemmatrix} \gls{lat:systemmatrix} vor jedem Prädikationsschritt neu berechnet werden.
...
\vdots \\
\gls{y}_{\gls{lat:LinienKOS}\gls{lat:naturalnumber}}
\end{pmatrix}
=
\begin{pmatrix}
\gls{lat:lateraloffset} +
...
\vdots \\
\gls{lat:lateraloffset} +
\gls{gre:yawangle} \cdot \gls{x}_{\gls{lat:LinienKOS}\gls{lat:naturalnumber}} +
\frac{1}{2} \cdot \gls{lat:curvature} \cdot \gls{x}_{\gls{lat:LinienKOS}\gls{lat:naturalnumber}}^2 +
\frac{1}{6} \cdot \gls{lat:curvaturechange} \cdot 
\gls{x}_{\gls{lat:LinienKOS}\gls{lat:naturalnumber}}^3  
\end{pmatrix} \\
& =
\begin{pmatrix}
...
1 & \gls{x}_{\gls{lat:LinienKOS}\gls{lat:naturalnumber}} & 
\frac{1}{2} \cdot \gls{x}_{\gls{lat:LinienKOS}\gls{lat:naturalnumber}}^2 &
\frac{1}{6} \cdot \gls{x}_{\gls{lat:LinienKOS}\gls{lat:naturalnumber}}^3
\end{pmatrix}
\cdot
\begin{pmatrix}
\gls{lat:lateraloffset} \\
...
\begin{enumerate}
\item Bildung der \glsdesc{lat:systemmatrix} \gls{lat:systemmatrix} mittels der seit dem letzten Bild gefahrenen Distanz \gls{lat:distpic}
